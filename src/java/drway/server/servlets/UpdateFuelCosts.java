 ...
 * @author MIRO
 */
public class UpdateFuelCosts extends HttpServlet {
    ...
    int message;
    JsonArray result;
    String appVer, osID, deviceId, deviceName, osVer, fuelTypeListVersion, userId;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("jsonFile") != null) {
                ...
                fuelTypeListVersion = jsonObject.get("fuelListVer").getAsString();
                //
                if (appVer.isEmpty() || osID.isEmpty() || deviceId.isEmpty() || deviceName.isEmpty() || osVer.isEmpty() || userId.isEmpty() || fuelTypeListVersion.isEmpty()) {
...
//                        userDao.blockUser(Integer.parseInt(userId));
                ...
                if (message == 0) {
                    ...
                    if (currentVersionDAO.getFuelCostListVersion() != Integer.parseInt(fuelTypeListVersion)) {
                        System.out.println("new version of FuelCosts: " + currentVersionDAO.getFuelCostListVersion());
                        JsonObject success = new JsonObject();
                        System.out.println("1");
                        FuelLookUpDAO fuelCostLookUpDAO = HibernateUtil.getFUEL_COST_LOOK_UP_DAO();
                        System.out.println("2");
                        success.addProperty("fuelListVer", currentVersionDAO.getFuelCostListVersion());
                        System.out.println("3");
                        for (FuelLookUp fuelTypeLookUp : fuelCostLookUpDAO.getList()) {
                            System.out.println("3.5");
                            JsonObject obj = new JsonObject();
                            obj.addProperty("id", fuelTypeLookUp.getId());
                            obj.addProperty("model", fuelTypeLookUp.getModel());
                            obj.addProperty("cost", fuelTypeLookUp.getValue());
                            result.add(obj);
                        }
                        ...
                        System.out.println("no change in FuelCosts");
                        JsonObject success = new JsonObject();
            ...
            System.out.println("End UpdateFuelCosts\n");
        } catch (NullPointerException ex) {
            System.out.println("NullPointerException (missing parameter most likely) in UpdateFuelCosts");
            ex.printStackTrace();
            ...
            System.out.println("Exception (NumberFormat most likely) in UpdateFuelCosts");
            ex.printStackTrace();
