 ...
 * @author MIRO
 */
public class UpdateCarList extends HttpServlet {
    ...
    int message;
    JsonArray result;
    String appVer, osID, deviceId, deviceName, osVer, carListVersion, userId;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("jsonFile") != null) {
                ...
                carListVersion = jsonObject.get("carListVer").getAsString();
                userId = jsonObject.get("userId").getAsString();

                //
                if (appVer.isEmpty() || osID.isEmpty() || deviceId.isEmpty() || deviceName.isEmpty() || osVer.isEmpty() || userId.isEmpty() || carListVersion.isEmpty()) {
...
//                        userDao.blockUser(Integer.parseInt(userId));
                ...
                if (message == 0) {
                    ...
                    if (currentVersionDAO.getCarListVersion() != Integer.parseInt(carListVersion)) {
                        System.out.println("new version of CarList: " + currentVersionDAO.getCarListVersion());
                        JsonObject success = new JsonObject();
                        CarListLookUpDAO carListLookUpDAO = HibernateUtil.getCAR_LIST_LOOK_UP_DAO();
                        success.addProperty("carListVer", currentVersionDAO.getCarListVersion());

                        for (CarListLookUp carListLookUp : carListLookUpDAO.getList()) {
                            JsonObject obj = new JsonObject();
                            obj.addProperty("id", carListLookUp.getCarId());
                            obj.addProperty("model", carListLookUp.getCarValue());
                            result.add(obj);
                        }
                        ...
                        System.out.println("no change in CarList");
                        JsonObject success = new JsonObject();
            ...
            System.out.println("End UpdateCarList\n");
        } catch (NullPointerException ex) {
            System.out.println("NullPointerException (missing parameter most likely) in UpdateCarList");
            ex.printStackTrace();
            ...
            System.out.println("Exception (NumberFormat most likely) in UpdateCarList");
            ex.printStackTrace();
